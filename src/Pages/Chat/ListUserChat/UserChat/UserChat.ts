...
import UserChatTmpl from './UserChat.tmpl';
import styles from './UserChat.module.less';

export const ChatUser = (avatar: string, name: string, time: string, lastMessage: string, newMessage: string) => {
  const template = Handlebars.compile(UserChatTmpl);
